...
@click.option(
    "--output-dir", "-o", type=str, help="Output directory for reports. Default: reports/"
)
...
@click.option(
    ...
    "-w",
    type=int,
    help="Max parallel workers for S3 downloads. Default: auto (2x CPU cores)",
)
@click.option(
    "--cache-dir",
    type=str,
    help="Directory for caching downloaded files. Default: ~/.cache/cur-reports",
)
@click.option(
    "--no-cache", is_flag=True, help="Disable local file caching (always download from S3)"
)
...
def generate_report(
    ...
    if sample_files is not None and sample_files <= 0:
        print(
            f"{Fore.RED}Error: --sample-files must be a positive integer, got {sample_files}{Style.RESET_ALL}"
        )
        sys.exit(1)

    if max_workers is not None and max_workers <= 0:
        print(
            f"{Fore.RED}Error: --max-workers must be a positive integer, got {max_workers}{Style.RESET_ALL}"
        )
        sys.exit(1)
    ...
    if start_dt > end_dt:
        print(
            f"{Fore.RED}Error: Start date ({start_dt.date()}) cannot be after end date ({end_dt.date()}){Style.RESET_ALL}"
        )
        sys.exit(1)
    ...
    try:
        ...
        if cur_data.is_empty():
            print(
                f"{Fore.RED}Error: No CUR data found for the specified date range{Style.RESET_ALL}"
            )
            sys.exit(1)
        ...
        # Get all analytics - using monthly trend methods for all visualizations
        processor.get_total_cost()
        service_trend = processor.get_cost_trend_by_service(top_services=top_n)
        account_trend = processor.get_cost_trend_by_account(top_accounts=top_n)
        ...
        region_trend = processor.get_cost_trend_by_region(top_regions=top_n)
        discounts_trend = processor.get_discounts_trend()
        discounts_by_service_trend = processor.get_discounts_by_service_trend(top_n=top_n)
        ...
        # For CSV exports only (totals, not monthly)
        ...
        # Generate 9 charts (all with monthly context)
        with tqdm(
            total=9, desc="Generating charts", bar_format="{l_bar}{bar}| {n_fmt}/{total_fmt}"
        ) as pbar:
            # Service and account trends (monthly breakdown)
            ...
            # Account-service heatmap (cross-dimensional view)
            ...
            # Anomaly detection (monthly with z-scores)
            visualizer.create_anomaly_chart(anomalies)
            pbar.update(1)

            # Region trends (monthly breakdown)
            if not region_trend.empty:
                visualizer.create_region_trend_chart(region_trend)
            pbar.update(1)

            # Discounts trend (monthly breakdown by type)
            if not discounts_trend.empty:
                visualizer.create_discounts_trend_chart(discounts_trend)
            pbar.update(1)

            # Discounts by service trend (monthly breakdown)
            if not discounts_by_service_trend.empty:
                visualizer.create_discounts_by_service_trend_chart(discounts_by_service_trend)
            pbar.update(1)

            # Savings Plan effectiveness trend (monthly breakdown)
            if not savings_plan_trend.empty:
                visualizer.create_savings_plan_trend_chart(savings_plan_trend)
            pbar.update(1)
        ...
        if summary_stats.get("total_discounts", 0) > 0:
            print(
                f"Discounts/Credits: {Fore.GREEN}-${summary_stats['total_discounts']:,.2f}{Style.RESET_ALL}"
            )
        ...
        if savings_plan_summary.get("total_savings", 0) > 0:
            ...
            print(
                f"  Total Savings: {Fore.GREEN}${savings_plan_summary['total_savings']:,.2f}{Style.RESET_ALL}"
            )
            print(
                f"  Savings Rate: {Fore.GREEN}{savings_plan_summary['savings_percentage']:.1f}%{Style.RESET_ALL}"
            )
